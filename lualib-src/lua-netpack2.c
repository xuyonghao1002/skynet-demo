...
/*
	Each package is uint24 + data , uint24 (serialized in big-endian) is the number of bytes comprising the data .
 */
...
push_more(lua_State *L, int fd, uint8_t *buffer, int size) {
	...
	if (size == 2)
	{
		struct uncomplete * uc = save_uncomplete(L, fd);
		uc->read = -2;
		uc->header = (int)buffer[1];
		uc->new_header = (int)buffer[0];
		return;
	}
	...
	if (uc) {
		...
		if (uc->read < 0) {
			...
			if (uc->read == -1) {
				...
				int pack_size = uc->header << 16 | (int)buffer[0] << 8 | (int)buffer[1];
				buffer += 2;
				size -= 2;
				uc->pack.size = pack_size;
				uc->pack.buffer = skynet_malloc(pack_size);
				...
				int pack_size = uc->new_header << 16 | uc->header << 8 | *buffer;
				++buffer;
				--size;
				uc->pack.size = pack_size;
				uc->pack.buffer = skynet_malloc(pack_size);
...
lpack(lua_State *L) {
	...
	uint8_t * buffer = skynet_malloc(len + 3);
	write_size(buffer, len);
	memcpy(buffer + 3, ptr, len);

	lua_pushlightuserdata(L, buffer);
	lua_pushinteger(L, len + 3);
